...
def plot_power_law_matplotlib(df, compare_distribution, xmin = None):
    # Fit the data to a power law distribution
    data = df['Degree'].values
    data = data[data > 0]
    ...
    fit = powerlaw.Fit(data, xmin=xmin, discrete=False)
...
def compare_distributions(df, distribution1, distribution2, xmin = None):
...
st.subheader('Complementary Cumulative Distribution Function (CCDF) plots with xmin as the minimum degree')
...
for idx, file_path in enumerate(file_paths):
    with tabs[idx]:
        if os.path.exists(file_path):
            ...
            try:
                fig = plot_power_law_matplotlib(df, selected_distribution, xmin = min_degree)
                st.pyplot(fig, use_container_width=False)

                # Perform and display the distribution comparison
                R, p = compare_distributions(df, 'power_law', selected_distribution, xmin = min_degree)
...
st.subheader('Complementary Cumulative Distribution Function (CCDF) plots with optimal xmin')
st.write("Select the threshold and the complementary cumulative distribution function (CCDF) plots will be displayed.")
